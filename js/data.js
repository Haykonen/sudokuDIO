/**
 * data.js
 * Dados do puzzle Sudoku — Projeto DIO
 *
 * Formato dos argumentos: col,row;valor,isFixed
 * isFixed = true  → célula dada (não pode ser editada)
 * isFixed = false → célula vazia (o jogador preenche)
 */
...
/**
 * Parseia a string do puzzle em duas matrizes 9×9:
 *   solution[row][col] → número correto (1–9)
 *   given[row][col]    → true se a célula é fixa
 */
function parsePuzzle(raw) {
  ...
  raw.trim().split(" ").forEach(token => {
    const [coords, valStr, fixedStr] = token.split(";");
    const [col, row] = coords.split(",").map(Number);
    solution[row][col] = parseInt(valStr, 10);
    given[row][col]    = fixedStr === "true";
  });

  return { solution, given };
}

const { solution: SOLUTION, given: GIVEN } = parsePuzzle(RAW_PUZZLE);